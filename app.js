...
module.exports = app => {
  ...
  app.beforeStart(async () => {
    ...
    await app.model.Admin.bulkCreate(data);
    console.log('生成模型和插入数据成功');

    // 初始化获取access_token：
    console.log('初始化获取access_token');
    const result = await app.curl(
      `https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid=${app.config.appid}&secret=${app.config.secret}`,
      {
        dataType: 'json',
      }
    );
